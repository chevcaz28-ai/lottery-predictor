...
"""
from predictor_core import markov_mc_method as seeded_markov_mc_method, freq_method, recency_method
...
def adaptive_weights(ss, game:str) -> Dict[str,float]:
    """
    Compute adaptive per-method weights for a given game using a recent rolling window
    of predictions from Prediction_Tracker.

    - Uses WEIGHTS_WINDOW_DAYS (env, default: 180) to limit history.
    - Requires at least WEIGHTS_MIN_SAMPLES (env, default: 30) predictions per method
      before trusting its score.
    - Treats legacy `markov1` rows as `markov_mc` so that historical performance from
      the old Markov implementation still contributes.
    """
    ws = get_or_create_worksheet(ss, "Prediction_Tracker", rows=20000, cols=len(TRACKER_COLS))
    ...
    try:
        window_days = int(os.getenv("WEIGHTS_WINDOW_DAYS", "180"))
    except Exception:
        window_days = 180
    cutoff_date = now_local().date() - dt.timedelta(days=window_days)
    ...
    for r in recs:
        ...
        # Prefer Next Draw Date; fall back to Timestamp if needed
        nd_str = normalize_date(r.get("Next Draw Date", "") or "")
        row_date = None
        if nd_str:
            try:
                row_date = dt.datetime.strptime(nd_str, "%Y-%m-%d").date()
            except Exception:
                row_date = None
        if row_date is None:
            ...
            if ts:
                try:
                    row_date = dt.datetime.strptime(ts[:10], "%Y-%m-%d").date()
                except Exception:
                    row_date = None
        if row_date is not None and row_date < cutoff_date:
            continue

        md = str(r.get("Method","") or "").strip()
        # Map legacy markov1 into markov_mc bucket
        if md == "markov1":
            md = "markov_mc"
        ...
        if md not in stats:
            stats[md] = {"cnt":0.0,"match_sum":0.0,"wins":0.0}
        stats[md]["cnt"] += 1.0
        stats[md]["match_sum"] += float(mc)
        stats[md]["wins"] += float(wc)

    weights: Dict[str,float] = {}

    try:
        min_samples = int(os.getenv("WEIGHTS_MIN_SAMPLES", "30"))
    except Exception:
        min_samples = 30

    for md, s in stats.items():
        if s["cnt"] < min_samples:
            # Not enough data to trust this signal; skip
            continue
        # Simple scoring: average matches + strong bonus for actual wins
    ...
    if total <= 0:
        # Fallback to uniform among whatever keys we have
        ks = list(weights.keys())
        return {k: 1.0/len(ks) for k in ks} if ks else {}
...
# Rolling window for predictor history: limit how many past draws each method sees.
try:
    history_window_draws = int(os.getenv("HISTORY_WINDOW_DRAWS", "300"))
except Exception:
    history_window_draws = 300
...
for game_label, merged_rows, next_draw in results:
    ...
    # Use only the most recent N draws as history for predictors
    rows_hist = merged_rows[:history_window_draws]

    if is_new or force:
        if is_new:
            # Use the full merged_rows[0] (latest result) for evaluation
