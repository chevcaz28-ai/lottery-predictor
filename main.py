...
# Optional backtest engine (keeps main.py runnable if module isn't present).
try:
    from backtest_engine import backtest_powerball_frequency_recency  # type: ignore
except Exception:
    backtest_powerball_frequency_recency = None  # type: ignore
...
def write_predictions_for_game(ss, game:str, rows_hist: List[List[Any]], next_draw_date:str):
    ...
    normalize_tracker(ws)
    # Methods that can be disabled via env (global or per-game)
    # Example: DISABLE_METHODS="baseline,llm" or DISABLE_METHODS_POWERBALL="llm"
    disabled = {m.lower() for m in disabled_methods_for_game(game)}
    ...
    # Enabled methods
    # Defaults kept "on" to preserve legacy behavior; workflow env vars can override.
    enable_baseline = env_true("ENABLE_BASELINE", True)
    enable_llm = env_true("ENABLE_LLM_METHOD", True)
    if enable_llm and not os.getenv("OPENAI_API_KEY", "").strip():
        enable_llm = False
    enabled_methods_count = (3 if enable_baseline else 0) + (1 if enable_llm else 0)
    if enabled_methods_count <= 0:
        print(f"[{game}] No prediction methods enabled; skipping predictions.")
        return
...
def main():
    ...
    if RUN_BACKTEST:
        bt_fn = globals().get("backtest_powerball_frequency_recency")
        wb_fn = globals().get("write_backtest")
        if callable(bt_fn):
            try:
                print("[BACKTEST] Enabled (RUN_BACKTEST=1). Running Powerball frequency+recency backtest (read-only by default).")
                bt = bt_fn(merged_pb, lookback=200)
                if WRITE_BACKTEST and callable(wb_fn):
                    wb_fn(ss, bt)
            except Exception as e:
                print(f"[BACKTEST] Failed safely (continuing normal run): {e}")
        else:
            print("[BACKTEST] RUN_BACKTEST=1 but backtest function is not defined; skipping.")
