...
# Fetch & parse (soft-fail: if API is down, skip predictions but keep heartbeat)
try:
    data = fetch_wi_results(rapid_key)
except Exception as e:
    print(f"[WARN] RapidAPI fetch failed: {e}. Skipping predictions this run.")
    # still write heartbeat and return
    return
