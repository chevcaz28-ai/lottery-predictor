...
def read_runlog(ss) -> dict:
    try:
        ws = ss.worksheet("RunLog")
    except Exception:
        print("[DIAG] RunLog sheet not found; starting empty.")
        return {}
    try:
    ...
    for row in rows[1:]:
        ...
        if not game:
            continue
        last_res = row[idx.get("LastResultDate",1)].strip() if len(row)>1 else ""
        last_pred = row[idx.get("LastPredictedNextDraw",2)].strip() if len(row)>2 else ""
        runlog[game] = {
            "LastResultDate": last_res,
            "LastPredictedNextDraw": last_pred,
        }
    return runlog

def write_runlog(ss, runlog: dict):
    ws = get_or_create_worksheet(ss, "RunLog", rows=max(len(runlog)+5,10), cols=3)
    rows = [["GameLabel","LastResultDate","LastPredictedNextDraw"]]
    for game, info in sorted(runlog.items()):
        rows.append([game, info.get("LastResultDate",""), info.get("LastPredictedNextDraw","")])
    try:
        ws.clear()
    except Exception:
        pass
