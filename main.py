...
def main():
    ...
    # Optional backtest for Powerball: frequency+recency hit rate + ranked tickets.
    if RUN_BACKTEST:
        try:
            print("[BACKTEST] Enabled (RUN_BACKTEST=1). Running Powerball frequency+recency backtest.")
            import pandas as pd
            from backtest_engine import run_backtest_frequency_recency, rank_tickets

            cols = RESULT_SCHEMAS["Powerball_Results"]  # Date, N1..N5, PB
            df_all = pd.DataFrame(merged_pb, columns=cols).copy()
            df_all["Date"] = pd.to_datetime(df_all["Date"], errors="coerce")

            # backtest_engine defaults to W1..W5, so rename N1..N5 -> W1..W5.
            rename_map = {f"N{i}": f"W{i}" for i in range(1, 6)}
            df_all = df_all.rename(columns=rename_map)
            list_cols = [f"W{i}" for i in range(1, 6)]
            pb_col = "PB"
            for c in list_cols + [pb_col]:
                df_all[c] = pd.to_numeric(df_all[c], errors="coerce")

            metrics = run_backtest_frequency_recency(
                df_all, list_cols=list_cols, pb_col=pb_col, lookback_draws=200, warmup=40, top_k=40
            )
            print(
                f"[BACKTEST] Done. avg_matches={metrics.avg_matches:.3f}  "
                f"p_ge3={metrics.p_ge3:.3%}  p_ge4={metrics.p_ge4:.3%}  "
                f"p_ge5={metrics.p_ge5:.3%}  p_pb={metrics.p_pb_match:.3%}  samples={metrics.n_samples}"
            )

            if WRITE_BACKTEST:
                # Write a compact report to a dedicated sheet.
                title = "Backtest_Powerball_FR"
                try:
                    ws_bt = ss.worksheet(title)
                    ws_bt.clear()
                except Exception:
                    ws_bt = ss.add_worksheet(title=title, rows=300, cols=20)
                ...
                summary = [
                    ...
                    ["Avg matches", float(metrics.avg_matches)],
                    ["P(matches>=3)", float(metrics.p_ge3)],
                    ["P(matches>=4)", float(metrics.p_ge4)],
                    ["P(matches>=5)", float(metrics.p_ge5)],
                    ["P(PB match)", float(metrics.p_pb_match)],
                ]
                ws_bt.update(values=summary, range_name="A1")

                # Show top ranked tickets for the most recent training window.
                train = df_all.dropna(subset=["Date"]).sort_values("Date").tail(200).copy()
                ranked = rank_tickets(train, top_k=40, candidates=3000, rng_seed=123)
                header = list_cols + [pb_col, "score"]
                rows = [header] + [list(whites) + [int(pb), float(score)] for (whites, pb, score) in ranked[:60]]
                ws_bt.update(values=rows, range_name="A15")
