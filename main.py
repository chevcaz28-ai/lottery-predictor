...
import os, json, datetime as dt, re, random, math, hashlib
...
# ---- Diversity / variation controls (default OFF; enable via env) ----
...
ENABLE_DIVERSITY = _env_bool("ENABLE_DIVERSITY", "0")

# Per-game similarity limits (how many mains may overlap with an existing set before we reject it)
# These defaults are conservative and should not change behavior unless ENABLE_DIVERSITY=1.
DIVERSITY_MAX_OVERLAP = {
    "Powerball": _env_int("DIVERSITY_MAX_OVERLAP_PB", 3),     # reject if overlap >= 4 of 5
    "Megabucks": _env_int("DIVERSITY_MAX_OVERLAP_MB", 4),     # reject if overlap >= 5 of 6
    "Super Cash": _env_int("DIVERSITY_MAX_OVERLAP_SC", 4),    # reject if overlap >= 5 of 6
    "Badger 5": _env_int("DIVERSITY_MAX_OVERLAP_B5", 3),      # reject if overlap >= 4 of 5
}

# Optional: if overlap equals max and special ball also matches, reject (PB only)
DIVERSITY_REJECT_ON_SAME_SPECIAL_AT_MAX = _env_bool("DIVERSITY_REJECT_ON_SAME_SPECIAL_AT_MAX", "0")

# ---- Markov seed controls (default keeps current behavior) ----
# MARKOV_SEED_MODE:
#   auto  -> current behavior (seed from latest draw date)
#   fixed -> use MARKOV_RANDOM_SEED as a fixed integer seed
#   draw  -> derive a per-draw seed from (MARKOV_RANDOM_SEED base + hash(latest draw date))
MARKOV_SEED_MODE = str(os.getenv("MARKOV_SEED_MODE", "auto")).strip().lower() or "auto"
...
def write_predictions_for_game(ss, game:str, rows_hist: List[List[Any]], next_draw_date:str):
    ...
    # Parsed (mains_set, special) for diversity gate
    existing_parsed = []
    if ENABLE_DIVERSITY:
        for r in existing_for_draw:
            try:
                p = str(r.get("Prediction","")).strip()
                mains, sp = parse_prediction_to_nums(game, p)
                existing_parsed.append((set(mains), sp))
            except Exception:
                continue
    written_parsed = []
    ...
    enabled_methods_count = 3 + (1 if os.getenv("ENABLE_BASELINE","0").lower() in ("1","true","yes") else 0)
    ...
    if os.getenv("ENABLE_BASELINE","0").lower() in ("1","true","yes"):
        methods["last_draw_baseline"] = (rows_hist[0][1:1+need] if rows_hist else [], (latest_special if (game=="Powerball") else None))
    ...
    # Markov per-draw seed mode (kept OFF by default via MARKOV_SEED_MODE=auto)
    markov_seed_mode = "auto"
    if MARKOV_SEED_MODE == "fixed":
        markov_seed_mode = MARKOV_RANDOM_SEED if str(MARKOV_RANDOM_SEED).strip().isdigit() else "auto"
    elif MARKOV_SEED_MODE == "draw":
        try:
            base_seed = int(str(MARKOV_RANDOM_SEED).strip()) if str(MARKOV_RANDOM_SEED).strip().isdigit() else 123
        except Exception:
            base_seed = 123
        latest_date_for_seed = str(rows_hist[0][0]) if rows_hist else today_local_str()
        h = hashlib.sha1(latest_date_for_seed.encode("utf-8")).hexdigest()
        markov_seed_mode = str((base_seed + int(h[:12], 16)) % (2**31 - 1))

    methods["markov_mc"] = (seeded_markov_mc_method(rows_hist, need, lo, hi, MARKOV_TEMP, MARKOV_SMOOTH, seed_mode=markov_seed_mode), None)
    ...
    def emit(md:str, nums:List[int], sp:Optional[int]):
        ...
        # --- Diversity gate (prevents near-duplicates / mode collapse) ---
        if ENABLE_DIVERSITY:
            try:
                max_overlap = int(DIVERSITY_MAX_OVERLAP.get(game, 3))
            except Exception:
                max_overlap = 3
            cand_set = set(int(x) for x in (nums or []) if str(x).strip().isdigit())
            for (prior_set, prior_sp) in (existing_parsed + written_parsed):
                overlap = len(cand_set & prior_set)
                if overlap > max_overlap:
                    return False
                if (game == "Powerball" and DIVERSITY_REJECT_ON_SAME_SPECIAL_AT_MAX and overlap == max_overlap):
                    try:
                        if special is not None and prior_sp is not None and int(special) == int(prior_sp):
                            return False
                    except Exception:
                        pass
        ...
        if ENABLE_DIVERSITY:
            try:
                written_parsed.append((set(int(x) for x in (nums or []) if str(x).strip().isdigit()), special))
            except Exception:
                pass
        return True
