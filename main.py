...
def adaptive_weights(ss, game:str) -> Dict[str,float]:
    """
    Compute per-method weights from historical prediction performance in Prediction_Tracker.

    IMPORTANT:
    - This is intentionally conservative: it blends learned weights with uniform weights,
      and caps dominance so one method cannot take over.
    - Kill switch: set ENABLE_METHOD_WEIGHTING=0 to disable (default is off).
    """
    if os.getenv("ENABLE_METHOD_WEIGHTING","0").lower() not in ("1","true","yes"):
        return {}
    ...
    lookback_rows = int(os.getenv("METHOD_WEIGHTING_LOOKBACK_ROWS","400") or "400")
    min_rows      = int(os.getenv("METHOD_WEIGHTING_MIN_ROWS","40") or "40")
    alpha         = float(os.getenv("METHOD_WEIGHTING_BLEND_ALPHA","0.35") or "0.35")  # 0=uniform, 1=fully learned
    cap_ratio     = float(os.getenv("METHOD_WEIGHTING_CAP_RATIO","2.0") or "2.0")      # max(weight)/min(weight)
    win_bonus     = float(os.getenv("METHOD_WEIGHTING_WIN_BONUS","5.0") or "5.0")      # multiplies Win Count

    # When computing weights we treat legacy `markov1` results as coming from `markov_mc`.
    allowed_methods = {
        "freq50",
        "recency",
        "markov_mc",
    ...
    # Filter to relevant game + recent rows (most recent rows are at the end of recs)
    game_recs = [r for r in recs if str(r.get("Game","")).strip()==game]
    if not game_recs:
        return {}
    game_recs = game_recs[-lookback_rows:]
    ...
    total_cnt = sum(v["cnt"] for v in stats.values())
    if total_cnt < float(min_rows):
        # Not enough evaluated predictions yet -> do not weight (falls back to uniform).
        return {}

    # Raw learned score: avg matches + (win_bonus * wins)
    raw = {}
    for md, s in stats.items():
        if s["cnt"] <= 0:
            continue
        raw_score = (s["match_sum"]/s["cnt"]) + win_bonus*s["wins"]
        raw[md] = max(0.0, float(raw_score))

    if not raw:
        return {}

    # Normalize learned weights over allowed_methods present
    present = list(raw.keys())
    learned_sum = sum(raw.values())
    if learned_sum <= 0:
        return {}

    learned = {k: raw[k]/learned_sum for k in present}
    uniform = {k: 1.0/len(present) for k in present}

    # Blend: conservative (prevents sudden behavior shifts)
    blended = {k: (1.0-alpha)*uniform[k] + alpha*learned[k] for k in present}

    # Cap dominance: enforce max/min ratio
    if cap_ratio and cap_ratio > 1.0:
        vals = list(blended.values())
        vmin = min(vals) if vals else 0.0
        vmax = max(vals) if vals else 0.0
        if vmin > 0 and vmax/vmin > cap_ratio:
            # compress by raising low weights and lowering high weights toward geometric mean
            target_min = vmax / cap_ratio
            for k,v in list(blended.items()):
                if v < target_min:
                    blended[k] = target_min
            # renormalize
