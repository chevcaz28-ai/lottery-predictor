...
"""
from predictor_core import freq_method, recency_method
...
def markov_mc_method(
    ...
    """
    Markov-chain based picker (Option A).

    - Treat each past draw as a full state (set of numbers).
    - Build a first-order transition model over numbers using the helper
      `build_transition_and_prior`.
    - Condition the next-number distribution on the most recent draw.
    - Sample without replacement to form each combination.
    - Add a mild diversity constraint so that we avoid emitting many
      nearly-identical combinations (sharing all but 1 number).
    """
    # Filter to valid numeric rows (most recent first).
    ...
    if not history:
        # Fallback: simple random sample from the full range.
        universe = list(range(lo, hi + 1))
        if len(universe) < need:
            picks = universe[:]
        else:
            picks = random.sample(universe, need)
        return [sorted(picks)]

    max_num = hi

    # Build Markov transition and prior over 1..max_num.
    T, prior = build_transition_and_prior(history, max_num, smooth)

    # Most recent draw as the conditioning "last state".
    last_set = sorted(set(int(x) for x in history[0] if isinstance(x, (int, float, str)) and str(x).strip()))
    probs = next_probs_from_last(last_set, T, prior, temp)

    # Candidate universe and RNG (optionally seeded).
    cands = list(range(lo, max_num + 1))
    seed_raw = os.getenv("MARKOV_RANDOM_SEED", "auto").strip().lower()
    rng = random.Random()
    if seed_raw and seed_raw != "auto":
        try:
            rng.seed(int(seed_raw))
        except ValueError:
            # Non-numeric seed: hash it for determinism.
            rng.seed(hash(seed_raw))
    else:
        # Auto: seed with current time & PID for enough variability.
        rng.seed(int(time.time() * 1000) ^ os.getpid())

    def sample_once() -> List[int]:
        """Single Markov-based combination."""
        picks = pick_weighted_without_replacement(cands, probs, need, rng)
        return sorted(picks)

    def is_too_similar(a: List[int], b: List[int]) -> bool:
        """Return True if two combos share all but 1 number (or more)."""
        sa = set(a)
        sb = set(b)
        same = len(sa & sb)
        return same >= max(len(sa), len(sb)) - 1

    predictions: List[List[int]] = []
    max_attempts = max(need * 20, 50)

    attempts = 0
    while len(predictions) < need and attempts < max_attempts:
        attempts += 1
        combo = sample_once()
        # Enforce mild diversity vs. already-chosen Markov combos.
        if any(is_too_similar(combo, prev) for prev in predictions):
            continue
        predictions.append(combo)

    # If we still came up short (extreme degeneracy), fill the rest
    # without the diversity constraint so that the caller always gets
    # `need` combinations.
    while len(predictions) < need:
        predictions.append(sample_once())

    return predictions


def load_best_methods(ss, game: str) -> Dict[str, float]:
    """
    Optional override/scale for method weights from a 'Best_Methods' sheet.

    Expected sheet structure (header row in A1:C1):
        Game | Method | Weight

    - Game   : string, e.g., "Powerball", "Megabucks", etc.
    - Method : one of last_draw_baseline, freq50, recency, markov_mc, llm_gpt, filler_random
    - Weight : positive float; rows with non-positive or invalid weights are ignored.

    The returned dict maps method -> normalized weight (sum to 1.0).
    These weights are *combined* with adaptive_weights:
        final_weight(method) = adaptive_weights(method) * sheet_weight(method)
    """
    ws = get_or_create_worksheet(ss, "Best_Methods", rows=100, cols=3)
    rows = ws.get_all_values()
    if not rows:
        # Initialize header for convenience
        ws.update(values=[["Game", "Method", "Weight"]], range_name="A1")
        return {}
    ...
    for r in records:
        ...
        if not m:
            continue
        try:
            w = float(str(r.get("Weight", "0")).strip())
        except Exception:
            continue
        if w <= 0:
            continue
        best[m] = best.get(m, 0.0) + w

    if not best:
        return {}

    total = sum(v for v in best.values() if v > 0)
    if total <= 0:
        return {}
...
def write_predictions_for_game(ss, game:str, rows_hist: List[List[Any]], next_draw_date:str):
    ...
    methods["markov_mc"] = (
    markov_mc_method(rows_hist, need, lo, hi, MARKOV_TEMP, MARKOV_SMOOTH),
    None,
)
    ...
    if remaining > 0:
        # Start from adaptive weights
        weights = adaptive_weights(ss, game) or {}

        # Optional: combine with Best_Methods sheet overrides, if present
        best = load_best_methods(ss, game)
        if best:
            combined: Dict[str, float] = {}
            # Scale adaptive weights by sheet weights (default 1.0 if not specified)
            for m in enabled_methods:
                aw = weights.get(m, 0.0)
                bw = best.get(m, 1.0)
                score = aw * bw
                if score > 0:
                    combined[m] = score
            # If adaptive had nothing useful, fall back to sheet-only for enabled methods
            if not combined:
                for m in enabled_methods:
                    bw = best.get(m, 0.0)
                    if bw > 0:
                        combined[m] = bw
            # Normalize combined weights if we have any
            if combined:
                total_c = sum(combined.values())
                if total_c > 0:
                    weights = {m: combined[m]/total_c for m in combined}
                else:
                    weights = {}
            else:
                weights = {}

        # If still empty at this point, fall back to uniform over enabled methods
        if not weights:
        ...
        def weighted_pick(w:Dict[str,float])->str:
            ...
            for k,v in w.items():
                ...
                if r <= cum:
                    return k
            # Fallback to first key if rounding issues
            return next(iter(w))
...
def evaluate_pending_predictions(ss, game_label, latest_draw_row):
    """Minimal stub so the workflow can run without failing.

    This currently does NOT score or update any rows in Prediction_Tracker.
    It only verifies that the sheet exists and logs a diagnostic message.
    Once we are satisfied the rest of the pipeline is stable, this function
    can be expanded to perform full evaluation logic.
    """
    try:
        print(f"[DIAG] evaluate_pending_predictions stub for game={game_label}")
        try:
            # Touch the tracker sheet so we fail here (and not later)
            ws = ss.worksheet("Prediction_Tracker")
        except Exception as e:
            # If the tracker sheet is missing, just warn and continue
            print(f"[WARN] Prediction_Tracker sheet not available for {game_label}: {e}")
            return
        # No-op for now: do not modify any cells, just return cleanly
        return
    except Exception as e:
        # Never let evaluation kill the whole run
        print(f"[WARN] evaluate_pending_predictions stub error for {game_label}: {e}")
        return
