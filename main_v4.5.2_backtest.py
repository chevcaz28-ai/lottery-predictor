...
# ---- Optional backtest (read-only by default; guarded by flags) ----
if RUN_BACKTEST:
    print("[BACKTEST] Enabled (RUN_BACKTEST=1). Running Powerball frequency+recency backtest (read-only by default).")
    try:
        from backtest_engine import run_backtest_frequency_recency, split_by_date, rank_tickets  # lazy import
        import pandas as pd

        pb_df = pd.DataFrame(
            merged_pb,
            columns=["Date","W1","W2","W3","W4","W5","PB"],
        )

        metrics = run_backtest_frequency_recency(pb_df)
        print(
            f"[BACKTEST][Powerball] draws={metrics.draws_tested} "
            f"avg_whites={metrics.avg_white_matches:.3f} "
            f"pb_rate={metrics.pb_hit_rate:.3%} "
            f"score={metrics.avg_total_score:.3f}"
        )

        train_df, _ = split_by_date(pb_df, split_ratio=0.5)
        ranked = rank_tickets(train_df, n_pool=1500)
        top5 = ranked[:5]

        print("[BACKTEST][Powerball] Top-5 ranked tickets:")
        for whites, pb, strength in top5:
            print(f"  {list(whites)} PB={pb} strength={strength:.1f}")

        if WRITE_BACKTEST:
            ws_bt = get_or_create_worksheet(ss, "Backtest_Results", rows=1000, cols=6)
            ws_bt.append_row(
                [
                    timestamp_local_str(),
                    "Powerball",
                    metrics.avg_white_matches,
                    metrics.pb_hit_rate,
                    metrics.avg_total_score,
                    "; ".join([f"{list(w)} PB={pb}" for w, pb, _ in top5]),
                ],
