...
def get_or_create(ss, title: str, rows: int = 5000, cols: int = 30):
    try:
...
def build_markov(
    ...
    # Build sequence by chaining draws in-order; also add cross-draw edges
    seq: List[int] = []
    for d in draws:
        d_sorted = sorted(int(x) for x in d if x)
        if not d_sorted:
            continue
        seq.extend(d_sorted)
    ...
    for x in seq:
        if 1 <= x <= max_num:
            prior[x] += 1.0
    prior = [p + laplace for p in prior]  # Laplace on prior too
...
def markov_pick(
    ...
    # Average transition probs from seeds in last draw
    avg = [0.0] * (max_num + 1)
    for seed in last:
    ...
    for _ in range(n_numbers):
        ...
        probs = softmax(scores[1:], temperature)
        # deterministic pick of highest prob to keep CI stable
...
GAME_CONFIG = {
    ...
    "Badger 5":  (5, 31),
    "Megabucks": (6, 49),
    "Powerball": (5, 69),  # PB separate; out-of-scope here
}
...
def read_history(ws_results) -> List[List[int]]:
    ...
    header = vals[0]
    rows = vals[1:]
    # find numeric columns by header tokens like N1,N2,... or plain numbers
    num_cols = []
    for i, h in enumerate(header):
        ...
        if h2.startswith("n") and any(ch.isdigit() for ch in h2):
            ...
            num_cols.append(i)
    if not num_cols:
        # fallback: assume columns 2..7 hold numbers
...
def run_for_game(ss, game: str, emit_count: int, temp: float, smooth: float, method_name: str, tzname: str):
    ...
    last = hist[0]  # assuming row 2 is newest (consistent with your sheet convention)

    combos = []
    for _ in range(emit_count):
        numbers_per_draw = n_numbers  # ← removed walrus operator
        picks = markov_pick(
            last,
            T,
            prior,
    ...
    next_draw = today_local_str(tzname)  # best-effort default
