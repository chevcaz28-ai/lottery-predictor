...
def write_predictions_to_tracker(ss, picks_rows, tzname, method_name):
    ...
    def pick_to_text(p):
        ...
        if isinstance(p, dict):
            # If caller already formatted a compact text in a known field, prefer that.
            for k in ("numbers", "pick", "prediction", "pred", "set", "combo", "text"):
                v = p.get(k)
                if isinstance(v, str) and v.strip():
                    return v.strip()

            main = p.get("main", None)
            pb = p.get("pb", p.get("powerball", None))
            mb = p.get("mb", p.get("mega", None))
            multi = p.get("multiplier", p.get("mult", None))
            cash = p.get("cash_ball", p.get("cash", None))
            ...
            main_s = _fmt_seq(main)

            # Build tail parts (Powerball-like, Mega-like, Badger5 etc.)
            tail_parts = []
            if pb is not None and str(pb).strip() != "":
                tail_parts.append(f"PB {int(pb)}")
            if mb is not None and str(mb).strip() != "":
                tail_parts.append(f"MB {int(mb)}")
            if cash is not None and str(cash).strip() != "":
                tail_parts.append(f"CB {int(cash)}")
            if multi is not None and str(multi).strip() != "":
                try:
                    tail_parts.append(f"x{int(multi)}")
                except Exception:
                    tail_parts.append(f"x{str(multi).strip()}")

            if tail_parts and main_s:
                return f"{main_s} | " + " ".join(tail_parts)
            if main_s:
                return main_s
            if tail_parts:
